...
## 進捗（いまここ）
- ✅ 直近で済んだこと：MVP一式＋Phase1の棋譜取り込みを本番公開。ログイン（合言葉）・本/章/局面・盤面エディタ・解説の自動保存・コメント（スレッド/リアルタイム/解決/未解決）・招待リンク・画像添付・PWA・**棋譜インポート(KIF/KI2/CSA)＆手順再生**。テスト計24件、RLS安全テスト12件すべて合格。
- 🟡 進行中：実機（iPhone/PC）での目視確認はユーザー側でこれから。
- 🔜 次の一歩：本人が本番で一通り試す。残りPhase1（画像注釈・盤バリデーション強化）やPhase2（閲覧専用リンク・通知・編集履歴・全文検索）へ。
